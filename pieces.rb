...
class Rook < Piece
  ...
  def find_possible_moves
    ...
    for i in (x+1)..7
      @possible_moves << [i,y] unless @active_player.pieces.any? { |piece| piece.position ==  [i,y] }
    end

    for i in (y+1)..7
      @possible_moves << [x,i] unless @active_player.pieces.any? { |piece| piece.position ==  [x,i] }
    end
